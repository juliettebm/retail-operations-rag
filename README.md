...
**Limites** : un hit-rate à 100% ne garantit pas une réponse juste. Sur une question test formulée de façon générique (« Quel est le délai de retour d'un article ? »), le passage qui répond directement n'est pas dans le contexte à k=2 ; un autre passage contient par coïncidence le même nombre, mais à propos d'un échange et non d'un retour, et le LLM retient quand même le mauvais chiffre, celui du passage sur les articles soldés (le notebook le montre explicitement). Le hit-rate lui-même est une approximation par mots-clés, pas un Recall@k annoté à la main : sur cette même question reformulée avec « article standard acheté en magasin », les mots-clés attendus suffisent à compter un hit et le modèle répond correctement. Le juge LLM-as-a-judge se contredit parfois lui-même (note 0/0 avec un commentaire disant la réponse correcte, sur 2 des 15 questions). Llama 3.2 (3B, local) est retenu pour le coût et le hors-ligne, pas pour la précision.
