...
**Limites** : un hit-rate à 100% ne garantit pas une réponse juste. Sur une question test formulée de façon générique (« Quel est le délai de retour d'un article ? »), le bon chiffre (30 jours, le délai standard aussi bien pour un retour que pour un échange chez Maison Kurt) est bien présent dans le contexte à k=2, via le passage sur l'échange standard ; le LLM répond quand même 14 jours, le chiffre du passage sur les articles soldés (le notebook le montre explicitement). Le hit-rate lui-même est une approximation par mots-clés, pas un Recall@k annoté à la main. Le juge LLM-as-a-judge se contredit parfois lui-même (note 0/0 avec un commentaire disant la réponse correcte, sur 2 des 15 questions). Llama 3.2 (3B, local) est retenu pour le coût et le hors-ligne, pas pour la précision.
